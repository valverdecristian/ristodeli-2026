...
export default function ChatMozoScreen() {
  ...
  const [numeroMesa, setNumeroMesa] = useState<string | null>(
    params.numeroMesa || null,
  );
  const [sesionId, setSesionId] = useState<string | null>(
    params.sesion_id || null,
  );
  ...
  const handleEnviarMensaje = async () => {
    ...
    try {
      ...
      // 🔔 Enviar notificación push al cliente (backup de la Edge function)
      try {
        const { data: asignacionList } = await supabase
          .from("lista_espera")
          .select("cliente_id")
        ...
        if (asignacionList && asignacionList.length > 0) {
          const clienteId = asignacionList[0]?.cliente_id;
          if (clienteId) {
            await NotificationService.notificarMensajeACliente(
              clienteId,
              textoAEnviar,
            );
          }
        ...
        console.warn("[chatMozo] Error enviando notificación:", notifError);
  ...
  return (
    <KeyboardAvoidingView
      ...
      className="flex-1 bg-primary pt-12"
    >
      <View className="flex-row items-center px-6 mb-4 justify-between">
        <View className="flex-row items-center">
          ...
          <View>
            ...
            <Text className="text-tertiary text-[10px] uppercase font-bold tracking-widest">
              Mesa N° {numeroMesa || "--"}
            </Text>
          </View>
        </View>
      ...
      <SafeAreaView
        edges={["bottom"]}
        className="flex-1 bg-secondary rounded-t-[32px] px-6 pt-6 pb-4 border-t border-tertiary/20"
      >
        {loading && mensajes.length === 0 ? (
          ...
          <FlatList
            ...
            ListEmptyComponent={
              <View className="flex-1 justify-center items-center pt-20">
                <Ionicons
                  ...
                  style={{ opacity: 0.3 }}
                />
                <Text className="text-primary/40 font-semibold text-xs mt-2 uppercase">
            ...
            renderItem={({ item }) => {
              ...
              return (
                <View
                  className={`flex-row ${esMio ? "justify-end" : "justify-start"} mb-4`}
                >
                  <View
                    className={`max-w-[80%] rounded-2xl px-4 py-3 shadow-sm ${
                      esMio
                        ? "bg-primary rounded-tr-none"
                        : "bg-tertiary/20 rounded-tl-none border border-tertiary/30"
                    }`}
                  >
                    <Text
                      className={`text-[10px] uppercase font-black mb-1 ${esMio ? "text-tertiary" : "text-primary"}`}
                    >
                      {esMio ? "Tú" : item.nombre_remitente}
                    </Text>
                    <Text
                      className={`text-sm font-medium ${esMio ? "text-white" : "text-primary"}`}
                    >
                      {item.mensaje}
                    </Text>
                    <Text
                      className={`text-[9px] mt-1 ${esMio ? "text-white/40 text-right" : "text-primary/40"}`}
                    >
        ...
        <View className="flex-row items-center pt-3 border-t border-primary/5">
          ...
          <TouchableOpacity
            ...
            className="bg-tertiary w-12 h-12 rounded-full items-center justify-center border-b-2 border-orange"
          >
