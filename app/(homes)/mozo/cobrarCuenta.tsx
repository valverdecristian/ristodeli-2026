...
export default function CobrarCuentaScreen() {
  ...
  const [mesaSeleccionada, setMesaSeleccionada] = useState<MesaCobro | null>(
    null,
  );
  ...
  const cargarMesasPendientes = async () => {
    try {
      ...
      showToast(
        "error",
        "Error al cargar",
        "No se pudieron obtener las mesas pendientes de cobro.",
      );
    } finally {
  ...
  const handleSeleccionarMesa = async (mesa: MesaCobro) => {
    try {
      ...
      productos?.forEach((p) =>
        mapPrecios.set(p.nombre.toLowerCase().trim(), p.precio),
      );
      ...
      (pedidos || []).forEach((p) => {
        if (p.categoria === "descuento") {
          ...
          const precio =
            mapPrecios.get(p.producto_nombre.toLowerCase().trim()) || 0;
      ...
      showToast(
        "error",
        "Error al cargar",
        "No pudimos obtener el detalle de la mesa.",
      );
    ...
    try {
      ...
      if (targetSesionId) {
        ...
        await AsyncStorage.removeItem(
          `ristodeli_juegos_sesion_${targetSesionId}`,
        );
      }
      ...
      showToast(
        "success",
        "Pago Confirmado",
        `La Mesa N° ${mesaSeleccionada.numero} ha sido cobrada y liberada con éxito.`,
      );
      ...
      NotificationService.notificarPagoConfirmado(
        ...
        console.warn("[cobrarCuenta] Fallo notificación pago al staff:", err),
      );
      ...
      showToast(
        "error",
        "Error al procesar",
        "No se pudo confirmar el pago de la mesa. Reintenta.",
      );
    } finally {
            ...
            >
              <View className="flex-row items-center">
                ...
                <View>
                  ...
                  <Text className="text-tertiary text-xs font-bold uppercase mt-1">
  ...
  return (
    <SafeAreaView className="flex-1 bg-primary">
      <LoadingModal
        visible={submittingPago}
        message="Confirmando Pago y Liberando..."
      />
      ...
      <View className="bg-tertiary px-6 pt-4 pb-5 flex-row items-center justify-between shadow-2xl">
        <View className="flex-row items-center flex-1">
          <TouchableOpacity
            onPress={() => router.back()}
            className="mr-3 p-1 active:opacity-75"
          >
            <Ionicons name="arrow-back" size={30} color="#31603D" />
          </TouchableOpacity>
          <View className="flex-1">
            <Text className="text-primary font-black text-2xl uppercase tracking-tighter leading-none">
              Cobrar Cuenta
            </Text>
            <Text className="text-primary/70 font-bold text-[9px] uppercase tracking-widest mt-1">
              Mesas Solicitantes
            </Text>
          </View>
        </View>
      ...
      {loadingMesas && mesas.length === 0 ? (
        <View className="flex-1 justify-center items-center bg-secondary">
          ...
          <Text className="text-primary font-bold mt-4 text-xs uppercase">
        ...
        <View className="flex-1 bg-secondary rounded-t-[32px] border-t border-tertiary/20 pt-6">
          {mesas.length === 0 ? (
            ...
            >
              ...
              <Text className="text-primary font-bold text-xs uppercase tracking-widest text-center">
                No hay mesas solicitando la cuenta
              </Text>
            </View>
          ) : (
            <>
      ...
      >
        <View className="flex-1 bg-black/60 justify-end">
          <SafeAreaView
            edges={["bottom"]}
            className="bg-secondary rounded-t-[35px] max-h-[85%] px-6 pt-6 pb-2 border-t-2 border-tertiary shadow-2xl"
          >
            {/* Encabezado */}
            <View className="flex-row justify-between items-center mb-4">
              <View>
                <Text className="text-primary font-black text-xl uppercase">
                  Cobro Mesa {mesaSeleccionada?.numero}
                </Text>
            ...
            {cargandoDetalles ? (
              ...
              >
                ...
                <View className="bg-secondary/40 border border-primary/10 rounded-2xl p-4 space-y-3">
                  <Text className="text-primary/70 font-bold text-xs uppercase tracking-widest border-b border-primary/5 pb-2">
                    Consumos
                  </Text>
                  {itemsConsumo.map((item) => (
                    <View
                      key={item.id}
                      className="flex-row justify-between items-center py-1 border-b border-primary/5"
                    >
                      <View className="flex-1">
                        <Text className="text-primary font-bold text-base">
                          {item.producto_nombre}
                        </Text>
                        <Text className="text-primary/50 text-xs font-bold uppercase">
                  ...
                  {itemsConsumo.length === 0 && (
                    <Text className="text-primary/40 text-center py-4 font-bold text-sm uppercase">
                ...
                <View className="bg-secondary/40 border border-primary/10 rounded-2xl p-4 space-y-2.5">
                  ...
                  {descuentoPorcentaje > 0 && (
                    <View className="flex-row justify-between items-center">
                      <View>
                        <Text className="text-emerald-600 text-sm font-semibold uppercase">
                          Descuento ({descuentoPorcentaje}%)
                        </Text>
                        <Text className="text-emerald-500/70 text-[11px] font-bold uppercase">
                          {descuentoInfo}
                        </Text>
                      </View>
                  ...
                  <View className="flex-row justify-between items-center">
                    <View>
                      ...
                      <Text className="text-primary/50 text-[11px] font-bold uppercase">
                        {propinaInfo || "Sin propina"}
                      </Text>
                    </View>
                    <Text className="text-primary font-black text-base">
