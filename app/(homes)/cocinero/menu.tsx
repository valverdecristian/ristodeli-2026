...
const AVAILABLE_HEIGHT = SCREEN_HEIGHT - HEADER_HEIGHT - TABS_HEIGHT - 120; // safe space calculation to avoid overflow/scroll
...
function TarjetaMenuCocina({ item, cardHeight }: { item: Producto; cardHeight: number }) {
                ...
                <View className="flex-1 w-full bg-white rounded-2xl overflow-hidden border border-secondary/15 relative justify-center items-center mt-4">
                    {item.fotos && item.fotos.length > 0 ? (
                        ...
                        <View className="items-center justify-center py-8">
                            <Ionicons name={item.tipo === 'postre' ? 'ice-cream-outline' : 'pizza-outline'} size={64} color="#F5C065" style={{ opacity: 0.3 }} />
...
export default function MenuCocineroScreen() {
    ...
    const cargarProductos = async () => {
        setLoading(true);
        try {
            const data = await ProductoService.obtenerPorCategorias([categoriaActual]);
            setProductos(data as Producto[]);
        } catch (error: any) {
            SoundService.reproducir('error');
            showToast("error", "Error de carga", error.message || "No se pudo obtener el catálogo.");
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <SafeAreaView className="flex-1 bg-primary">
            ...
            <View className="bg-tertiary px-6 pt-4 pb-5 flex-row items-center justify-between shadow-2xl">
                <View className="flex-row items-center flex-1">
                    ...
                    <View className="flex-1">
                        <Text className="text-primary font-black text-2xl uppercase tracking-tighter leading-none">Sector Cocina</Text>
                        <Text className="text-primary/70 font-bold text-[9px] uppercase tracking-widest mt-1">Catálogo de Productos</Text>
                    </View>
                </View>
            </View>
            ...
            <View className="flex-1 bg-secondary rounded-t-[32px] border-t border-tertiary/20 pt-6">
                ...
                {/* SELECTOR DE TABS */}
                <View className="flex-row justify-between px-6 mb-4" style={{ height: 48 }}>
                    <TouchableOpacity
                        onPress={() => setCategoriaActual('plato')}
                        className={`w-[48%] py-2.5 rounded-full border-b-4 ${categoriaActual === 'plato' ? 'bg-tertiary border-orange' : 'bg-primary border-primary/20'} active:opacity-90`}
                    >
                        <Text className={`text-center font-bold uppercase text-sm ${categoriaActual === 'plato' ? 'text-primary' : 'text-secondary'}`}>
                            Ver Platos
                        </Text>
                    </TouchableOpacity>
                    ...
                    <TouchableOpacity
                        ...
                        className={`w-[48%] py-2.5 rounded-full border-b-4 ${categoriaActual === 'postre' ? 'bg-tertiary border-orange' : 'bg-primary border-primary/20'} active:opacity-90`}
                    >
                        <Text className={`text-center font-bold uppercase text-sm ${categoriaActual === 'postre' ? 'text-primary' : 'text-secondary'}`}>
                            Ver Postres
                        </Text>
                    </TouchableOpacity>
                </View>
                ...
                {loading ? (
                    ...
                    <View className="flex-1 justify-center items-center px-8">
                        <Ionicons name={categoriaActual === 'postre' ? 'ice-cream-outline' : 'pizza-outline'} size={72} color="#31603D" style={{ opacity: 0.4 }} />
                        <Text className="text-primary font-black text-xl uppercase tracking-tighter text-center mt-4">
                            Sin {categoriaActual === 'postre' ? 'Postres' : 'Platos'}
                        </Text>
                        <Text className="text-primary/60 font-medium text-sm text-center mt-2 leading-relaxed">
                            No se encontraron {categoriaActual === 'postre' ? 'postres' : 'platos'} registrados en la carta.
                        </Text>
                    </View>
                ) : (
                    <FlatList
                        ...
                        renderItem={({ item }) => (
                            <TarjetaMenuCocina item={item} cardHeight={AVAILABLE_HEIGHT - 32} />
                        )}
                    />
                )}
