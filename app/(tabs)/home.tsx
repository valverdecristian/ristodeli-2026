...
export default function HomeEntradaScreen() {
    ...
    const { usuarioId, usuarioNombre, usuarioFoto, clienteId, nombre } = useLocalSearchParams();
    ...
    const currentNombre = String(isAnonimo ? nombre : (usuarioNombre || 'Cliente Express'));
    const currentFoto = String(usuarioFoto || '');
        ...
        try {
            if (isAnonimo) {
                ...
                router.replace({
                    ...
                    params: { usuarioNombre: currentNombre, usuarioFoto: currentFoto, usuarioId: currentId }
                });
            }
        } catch (error: any) {
            SoundService.reproducir('error');
