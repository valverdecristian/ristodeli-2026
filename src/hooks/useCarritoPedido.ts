...
export function useCarritoPedido() {
    ...
    // 💰 IMPORTE ACUMULADO VISIBLE EN TODO MOMENTO (Punto 12)
    const importeTotal = useMemo(() => {
        return carrito.reduce((total, item) => total + (item.precio * item.cantidad), 0);
    }, [carrito]);

    // ⏱️ TIEMPO MÁXIMO ESTIMADO DE ELABORACIÓN (Punto 12)
    const tiempoEstimadoMaximo = useMemo(() => {
        if (carrito.length === 0) return 0;
        // Se toma el tiempo de elaboración del plato que más tarde, no la sumatoria
